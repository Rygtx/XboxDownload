    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            if (File.Exists(Application.StartupPath + "\\" + UpdateFile.dataFile))
            {
                try
                {
                    using (FileStream stream = new FileStream(Application.StartupPath + "\\" + UpdateFile.dataFile, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        BinaryFormatter bFormat = new BinaryFormatter();
                        XboxGameDownload.dicXboxGame = (ConcurrentDictionary<String, XboxGameDownload.Products>)bFormat.Deserialize(stream);
                        stream.Close();
                    }
                }
                catch { }
        ...
        private void TsmiStartup_Click(object sender, EventArgs e)
        {
            if (!File.Exists(Application.StartupPath + "\\Interop.TaskScheduler.dll"))
            {
                ...
                MessageBox.Show("缺少所需的文件，正在下载，请稍候再执行此操作。", "请稍候", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private void TsmProductManual_Click(object sender, EventArgs e)
        {
            ...
            if (!fi.Exists)
            {
                ...
                Task task = Task.Run(() => { UpdateFile.Download(fi.Name); });
        ...
        private void TabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (tabControl1.SelectedTab.Name)
            {
                case "tabGames":
                    ...
                    if (flpGameWithGold.Controls.Count == 0 || dicGamesWithGold.Count == 0 || lbFreePlayDays == null)
                    {
        ...
        {
            FormNSHomepage dialog = new FormNSHomepage();
            dialog.ShowDialog();
            dialog.Dispose();
        }
        ...
        private void AddHosts(bool add)
        {
            if (add)
            {
                ...
                foreach (DataRow dr in dtHosts.Rows)
                {
                    ...
                    if (Convert.ToBoolean(dr["Enable"]) && !string.IsNullOrEmpty(hostName) && !DnsListen.dicHosts.ContainsKey(hostName) && DnsListen.reHosts.IsMatch(hostName) && IPAddress.TryParse(dr["IPv4"].ToString().Trim(), out IPAddress ip))
                    {
            ...
            if (add && Properties.Settings.Default.SetDns)
            {
                ...
                foreach (string str in sb.ToString().Split('\n'))
                {
                    ...
                    while (result.Success)
                    {
                        ...
                        if (DnsListen.reHosts.IsMatch(hostName) && IPAddress.TryParse(result.Groups["ip"].Value, out IPAddress ip))
                        {
        ...
        private void LvLog_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                {
                    tsmCopyLog.Visible = tsmExportLog.Visible = true;
                ...
                case "dlassets2.xboxlive.com":
                    {
                        ...
                        {
                            {"光环:无限(PC)", "513710f5-ab8e-4d7c-9ed5-d0ba94dcfb33" },
                            {"极限竞速:地平线5(PC)", "3d263e92-93cd-4f9b-90c7-5438150cecbf" },
                            {"战争机器5(PC)", "1e66a3e7-2f7b-461c-9f46-3ee0aec64b8c"}
                        };
                        for (int i = 0; i <= games.GetLength(0) - 1; i++)
                        {
                            ...
                            {
                                string url = XboxGame.Url.Replace(".xboxlive.cn", ".xboxlive.com");
                ...
                case "tlu.dl.delivery.mp.microsoft.com":
                    {
                        ...
                        {
                            Tag = "0e0dcbc6-1a24-42cf-847a-056d6d25f7ef|appx|9NBLGGH537BL",
                            Text = "我的世界(PC)",
                            AutoSize = true,
                            Parent = this.flpTestUrl
                        };
        ...
        private void LinkTestUrl_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            else if (Regex.IsMatch(url, @"^\w{8}-\w{4}-\w{4}-\w{4}-\w{12}\|"))
            {
                ...
                string extension = product[1];
                ThreadPool.QueueUserWorkItem(delegate { GetAppPackage(wuCategoryId, extension); });
            }
        }

        private void GetAppPackage(string wuCategoryId, string extension)
        {
            SetTextBox(tbDlUrl, "正在获取下载链接，请稍候...");
            string url = string.Empty;
            for (int i = 0; i <= 2; i++)
            {
                SocketPackage socketPackage = ClassWeb.HttpRequest(UpdateFile.homePage + "/Game/GetAppPackage?WuCategoryId=" + wuCategoryId, "GET", null, null, true, false, true, null, null, new String[] { "X-Organization: XboxDownload", "X-Author: Devil" }, null, null, null, null, null, 0, null, 5000, 5000);
                JavaScriptSerializer js = new JavaScriptSerializer();
                try
                {
                    var json = js.Deserialize<XboxPackage.App>(socketPackage.Html);
                    if (json != null && json.Code != null && json.Code == "200")
                    {
                        url = json.Data.Where(x => x.Name.ToLower().EndsWith("." + extension.ToLower())).Select(x => x.Url).FirstOrDefault();
                    }
                }
                catch { }
        ...
        private void SpeedTest(List<DataGridViewRow> ls, bool skipTimeOut = false)
        {
            ...
            //if (Form1.debug) headers = new string[] { "Range: bytes=0-1048575" }; //1M
            ...
            if (!Regex.IsMatch(tbDlUrl.Text, @"^https?://") && flpTestUrl.Controls.Count >= 1)
            {
                ...
                else if (Regex.IsMatch(url, @"^\w{8}-\w{4}-\w{4}-\w{4}-\w{12}\|"))
                {
                    ...
                    GetAppPackage(wuCategoryId, extension);
                    url = tbDlUrl.Text;
                }
            }
        ...
        private void DgvHosts_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            switch (dgvHosts.Columns[e.ColumnIndex].Name)
            {
                case "Col_HostName":
                    ...
                    {
                        if (!DnsListen.reHosts.IsMatch(e.FormattedValue.ToString().Trim()))
                        {
                            e.Cancel = true;
        ...
        private void CbHosts_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (result.Success)
            {
                ...
                else
                {
                    ...
                    {
                        case 1:
                            dnsServer = "1.12.12.12";  //doh.pub
                            break;
                        case 2:
                            dnsServer = "180.163.249.75";   //doh.360.cn
                            break;
                        case 3:
                            dnsServer = "8.8.8.8";
                            break;
        ...
        private void LinkCdnSpeedTest_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (item.ToString().Contains("Akamai"))
                    {
        ...
        readonly ConcurrentDictionary<String, string[]> dicGamesWithGold = new ConcurrentDictionary<String, string[]>();
        ...
        private void GameWithGold()
        {
            ...
            tasks[1] = new Task(() =>
            {
                ...
                {
                    SocketPackage socketPackage = ClassWeb.HttpRequest("https://news.xbox.com/en-us/tag/free-play-days/", "GET", null, null, true, false, true, null, null, null, ClassWeb.useragent, null, null, null, null, 0, null);
                    Match result = Regex.Match(socketPackage.Html, @"<h3 class=""feed__title"">\r?\n\s+<a href=""(?<url>https://news\.xbox\.com/en-us/(?<date>\d{4}/\d{2}/\d{2})/[^""]+)"">(?<title>[^<]+)</a>");
                    if (result.Success)
                    {
                        string url = result.Groups["url"].Value.Trim();
                        string title = HttpUtility.HtmlDecode(result.Groups["title"].Value).Trim();
                        ...
                        lbFreePlayDays = new LinkLabel
                        {
                            ...
                            AutoSize = true,
                            Visible = DateTime.Compare(dt.AddDays(3), DateTime.Now) >= 0
                        };
            ...
            if (dicGamesWithGold.Count >= 1 || lbFreePlayDays != null)
            {
                this.Invoke(new Action(() =>
                {
                    flpGameWithGold.Controls.Clear();
        ...
        private void StoreParse(Market market, ClassGame.Game json, int index, string language)
        {
            ...
            if (localizedPropertie != null && localizedPropertie.Count >= 1)
            {
                ...
                if (!string.IsNullOrEmpty(imageUri))
                {
                    try
                    {
                        pbGame.LoadAsync("http:" + imageUri + "?w=170&h=170");
                    }
                    catch { }
                }
            }
                        ...
                        if (displaySkuAvailabilitie.Sku.Properties.Packages != null)
                        {
                            foreach (var packages in displaySkuAvailabilitie.Sku.Properties.Packages)
                            {
                                ...
                                if (platformDependencie != null && packages.PlatformDependencies.Count >= 1)
                                {
                                    ...
                                    string url = (packageDownloadUris != null && packageDownloadUris.Count >= 1) ? packageDownloadUris[0].Uri : null;
                                    if (url == "https://productingestionbin1.blob.core.windows.net") url = null;
                                                ...
                                                default:
                                                    {
                                                        ...
                                                        string key = filename.Replace(version, "");
                                                        ...
                                                        if (item == null)
                                                        {
                                                            ...
                                                            if (dicAppPackage.TryGetValue(filename.ToLower(), out AppPackage appPackage) && (DateTime.Now - appPackage.Date).TotalSeconds <= 300)
                                                            {
                                                        ...
                                                        {
                                                            if (new Version(version) > new Version(item.SubItems[1].Tag.ToString()))
                                                            {
                            ...
                            List<ListViewItem> lsAppItem = lsDownloadUrl.ToArray().Where(x => x.Tag.ToString() == "App" && !x.SubItems[3].Text.StartsWith("http")).ToList();
                            if (lsAppItem.Count >= 1)
                            {
                                ThreadPool.QueueUserWorkItem(delegate { GetAppPackage(wuCategoryId, lsAppItem); });
                            }
                        }
            ...
            if (bundledId.Count >= 1 && json.Products.Count == 1)
            {
                ...
                {
                    JavaScriptSerializer js = new JavaScriptSerializer();
                    if (socketPackage.Html.Length > js.MaxJsonLength)
                        js.MaxJsonLength = socketPackage.Html.Length;
        ...
        readonly ConcurrentDictionary<string, DateTime> dicGetGamePackage = new ConcurrentDictionary<string, DateTime>();
        ...
        private void GetGamePackage(ListViewItem item, string contentId, string key, int platform, ClassGame.Packages packages)
        {
            ...
            {
                SocketPackage socketPackage = ClassWeb.HttpRequest(UpdateFile.homePage + "/Game/GetGamePackage?contentId=" + contentId + "&platform=" + platform, "GET", null, null, true, false, true, null, null, new String[] { "X-Organization: XboxDownload", "X-Author: Devil" }, null, null, null, null, null, 0, null, 5000, 5000);
                JavaScriptSerializer js = new JavaScriptSerializer();
                try
                {
                    json = js.Deserialize<XboxPackage.Game>(socketPackage.Html);
                }
                catch { }
            }
            bool succeed = false;
            if (json != null && json.Code == "200")
            {
                ...
                if(json.Data != null)
                {
                    Version version = new Version(Regex.Match(json.Data.Url, @"(\d+\.\d+\.\d+\.\d+)").Value);
                    bool update = false;
                    if (XboxGameDownload.dicXboxGame.TryGetValue(key, out XboxGameDownload.Products XboxGame))
                    {
            ...
            if (!succeed && platform == 2)
            {
                ...
                {
                    UpdateGamePackage(item, contentId, packages);
                }
                else
                {
                    this.Invoke(new Action(() =>
                    {
                        item.SubItems[3].Text = "授权已失效，请使用监听方式打开Xbox app(右键启动)随便下载一个游戏，等待日志显示下载地址即可更新授权。";
                    }));
                }
            }
        }

        private void UpdateGamePackage(ListViewItem item, string contentId, ClassGame.Packages packages)
        {
            string hosts = "packagespc.xboxlive.com", url = String.Empty;
            ulong filesize = 0;
            if (!DnsListen.dicHosts2.TryGetValue(hosts, out IPAddress ip))
            {
                if (IPAddress.TryParse(ClassDNS.DoH(hosts), out ip))
                {
                    DnsListen.dicHosts2.AddOrUpdate(hosts, ip, (oldkey, oldvalue) => ip);
                }
            }
            Uri uri = new Uri("https://" + hosts + "/GetBasePackage/" + contentId);
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("GET /GetBasePackage/" + contentId + " HTTP/1.1");
            sb.AppendLine("Connection: Keep-Alive");
            sb.AppendLine("Authorization: " + Properties.Settings.Default.Authorization);
            sb.AppendLine("User-Agent: libHttpClient/1.0.0.0");
            sb.AppendLine("Host: " + hosts);
            sb.AppendLine();
            SocketPackage socketPackage = ClassWeb.SslRequest(uri, Encoding.ASCII.GetBytes(sb.ToString()), ip, true);
            if (socketPackage.Headers.Contains("Unauthorized"))
            {
                Properties.Settings.Default.Authorization = null;
                Properties.Settings.Default.Save();
                url = "授权已失效，请使用监听方式打开Xbox app(右键启动)随便下载一个游戏，等待日志显示下载地址即可更新授权。";
            }
            else
            {
                if (Regex.IsMatch(socketPackage.Html, @"^{.+}$"))
                {
                    ...
                    try
                    {
                        var json = js.Deserialize<XboxGameDownload.Game>(socketPackage.Html);
                    ...
                    if (packageFiles != null)
                    {
                        ...
                        if (result.Success)
                            ...
                            version = new Version();
                        XboxGameDownload.Products XboxGame = new XboxGameDownload.Products
                        {
                            Version = version,
            ...
            if (Regex.IsMatch(url, @"^https?://")) ClassWeb.HttpRequest(UpdateFile.homePage + "/Game/AddGameUrl?url=" + ClassWeb.UrlEncode(url), "PUT", null, null, true, false, true, null, null, new String[] { "X-Organization: XboxDownload", "X-Author: Devil" }, null, null, null, null, null, 0, null);
        }

        readonly ConcurrentDictionary<string, AppPackage> dicAppPackage = new ConcurrentDictionary<string, AppPackage>();
        class AppPackage
        {
        ...
        private void GetAppPackage(string wuCategoryId, List<ListViewItem> lsAppItem)
        {
            SocketPackage socketPackage = ClassWeb.HttpRequest(UpdateFile.homePage + "/Game/GetAppPackage?WuCategoryId=" + wuCategoryId, "GET", null, null, true, false, true, null, null, new String[] { "X-Organization: XboxDownload", "X-Author: Devil" }, null, null, null, null, null, 0, null, 5000, 5000);
            JavaScriptSerializer js = new JavaScriptSerializer();
            ...
            try
            {
                json = js.Deserialize<XboxPackage.App>(socketPackage.Html);
            }
            catch { }
            ...
            this.Invoke(new Action(() =>
            {
                foreach (var item in lsAppItem)
                {
                    string url = String.Empty;
                    if (json != null && json.Code != null && json.Code == "200")
                    {
                        url = json.Data.Where(x => x.Name.ToLower() == item.SubItems[3].Tag.ToString().ToLower()).Select(x => x.Url).FirstOrDefault();
                        if (string.IsNullOrEmpty(url))
                        {
                            var data = json.Data.Where(x => Regex.Replace(x.Name, @"\d+\.\d+\.\d+\.\d+", "").ToLower() == item.SubItems[2].Tag.ToString().ToLower()).FirstOrDefault();
                            if (data != null)
                            {
        ...
        private void LvGame_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                {
                    if (Regex.IsMatch(lvGame.SelectedItems[0].SubItems[3].Text, @"^https?://"))
                    {
                        ...
                        tsmCopyUrl3.Visible = Regex.IsMatch(lvGame.SelectedItems[0].SubItems[3].Text, @"\.xboxlive\.cn");
                        tsmAuthorization.Visible = false;
                        contextMenuStrip2.Show(MousePosition.X, MousePosition.Y);
                    }
                    else if (Regex.IsMatch(lvGame.SelectedItems[0].SubItems[3].Text, @"^授权已失效"))
                    {
                        tsmCopyUrl1.Visible = tsmCopyUrl2.Visible = tsmCopyUrl3.Visible = false;
        ...
        private void TsmAuthorization_Click(object sender, EventArgs e)
        {
            ...
            {
                ToolStripMenuItem tsmi = (ToolStripMenuItem)sender;
                Process.Start("msxbox://game/?productId=" + (tsmi.Tag ?? lvGame.SelectedItems[0].Tag));
            }
            else
            {
        ...
        private void ButAppxInstall_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("路径或文件名不能包含中文。", "不支持中文路径或文件名", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            */
            ...
            try
            {
                using (Process p = new Process())
                {
                    ...
                    p.StartInfo.Arguments = "-noexit \"Add-AppxPackage -Path '" + filepath + "' -Volume '" + cbAppxDrive.Text + "'\"";
                    //p.StartInfo.Arguments = "-noexit \"cd '" + Path.GetDirectoryName(filepath) + "'\" \n \"Add-AppxPackage -Path '.\\" + Path.GetFileName(filepath) + "' -Volume '" + cbAppxDrive.Text + "'\"";
                    p.Start();
                }
            }
            catch (Exception ex)
        ...
        {
            SocketPackage socketPackage = ClassWeb.HttpRequest(UpdateFile.homePage + "/Game/GetAppPackage?WuCategoryId=f2ea4abe-4e1e-48ff-9022-a8a758303181", "GET", null, null, true, false, true, null, null, new String[] { "X-Organization: XboxDownload", "X-Author: Devil" }, null, null, null, null, null, 0, null, 5000, 5000);
            JavaScriptSerializer js = new JavaScriptSerializer();
            ...
            if (data != null)
            {
                ...
                socketPackage = ClassWeb.HttpRequest(data.Url, "GET", null, null, true, false, false, null, null, null, ClassWeb.useragent, null, null, null, null, 0, null);
                if (string.IsNullOrEmpty(socketPackage.Err) && socketPackage.Buffer.Length > 0 && socketPackage.Headers.Contains(" 200 OK") && socketPackage.Headers.IndexOf(data.Name, StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    try
                    {
        ...
        private void LinkResetWinsock_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (MessageBox.Show("请确认是否要“重置 Winsock 目录”？\n\n重置完成后需要重启计算机。", "重置Winsock目录", MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                outputString = outputString.Substring(outputString.IndexOf("netsh int ip reset") + 18);
